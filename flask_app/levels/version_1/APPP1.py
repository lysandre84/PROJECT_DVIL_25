...
app = Flask(
    __name__,
    template_folder="/home/Ad_@min/flask_app/levels/version_1/template",
    static_folder="/home/Ad_@min/flask_app/levels/version_1/static"
)
...
# -------------------------------------------------------------------------------------,
# ENDPOINT VULNÉRABLE POUR LE CHALLENGE (Niveau 1),
# -------------------------------------------------------------------------------------,
@app.route("/v1/unlock_public", methods=["POST"])
def vuln_unlock_public():
    """
    Faille de contrôle d'accès : aucun mécanisme d'authentification.
    Cet endpoint permet à n'importe qui de déverrouiller la serrure.
    """
    mqtt_client.publish(MQTT_TOPIC_UNLOCK, "deverouillage")
    lock_state["locked"] = False
    logging.warning("[VULN] Déverrouillage public via /v1/unlock_public !")
    return jsonify({"status": "unlocked", "message": "Serrure déverrouillée (faille niveau 1)"})
# -------------------------------------------------------------------------------------
